...
void hybrid_hc_ps::operator()(deme_t& deme,
                               const instance& init_inst,
                               const iscorer_base& iscorer,
                               unsigned max_evals,
                               time_t max_time)
{
    logger().debug("Hybrid Local Search Optimization");
    ...
    // Transform the field_set to discrete only
...
size_t hybrid_hc_ps::estimate_neighborhood(size_t distance,
                                            const field_set& fields)
{
    ...
    if (nn_estimate < distance) {
        logger().warn("hybrid_hc_ps::estimate_neighborhood : "
...
size_t hybrid_hc_ps::n_new_instances(size_t distance, unsigned max_evals,
...
size_t hybrid_hc_ps::cross_top_one(deme_t& deme,
                                    size_t deme_size,
                                    size_t num_to_make,
                                    size_t sample_start,
                                    size_t sample_size,
...
size_t hybrid_hc_ps::cross_top_two(deme_t& deme,
                                    size_t deme_size,
                                    size_t num_to_make,
                                    size_t sample_start,
                                    size_t sample_size,
...
size_t hybrid_hc_ps::cross_top_three(deme_t& deme,
                                      size_t deme_size,
                                      size_t num_to_make,
                                      size_t sample_start,
                                      size_t sample_size,
...
size_t hybrid_hc_ps::crossover(deme_t& deme, size_t deme_size,
...
size_t hybrid_hc_ps::resize_by_score(deme_t& deme, score_t cutoff)
{
    size_t ndeleted = 0;
    while (true) {
         auto first = deme.end();
...
/// so we want to keep it at some reasonably trim level.
//
bool hybrid_hc_ps::resize_deme(deme_t& deme, score_t best_score)
{
...
{
    logger().info() << hc_params.prefix_stat_deme << "Hybrid: # "   /* Legend for graph stats */
        "demeID\t"
        "iteration\t"
        "total_steps\t"
        "total_evals\t"
